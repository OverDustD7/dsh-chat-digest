...
(function () {
  ...
  function openNativeSession(id, epoch) {
    ...
    navTimers.push(setTimeout(function () {
      ...
      // A33：守卫从内存变量改成 **localStorage**（`claimWake`）——内存变量活不过一次刷新，
      //   而每刷新一次就再灌一整份提示词进对话，主 agent 就白跑一轮、白烧 token。
      if (claimWake(id)) return
      wokeFor = String(id)
  ...
  function renderPanel() {
    try {
      if (window.__cfwRight && listEl !== null) {
        // A32（2026-09-27）：`mounted` 守卫不能只看"挂过没有"，还要看**挂的是不是当前这个列表节点**。
        //   面板节点一旦被重建（A31 起，`ensurePanel()` 在"面板不存在 / 已脱离 DOM"时会新建一份），
        //   `listEl` 就换人了；而 right.js 在 `mount()` 时把 `listNode` **取一次值**存住 ——
        //   不重挂的话它会一直往已经脱离 DOM 的旧节点上画：屏幕那一栏要么空白、要么点了没反应。
        //   （同类缺陷出现过一次：热重载后新的 right.js 永远不被 mount。）
        if (!renderPanel.mounted || renderPanel.node !== listEl) {
          renderPanel.mounted = true
          renderPanel.node = listEl
  ...
  var wokeFor = ''   // 已经为哪个会话发过"唤醒"消息（同一个只发一次；A33 起真正的守卫是 localStorage）
  // A33（2026-09-27）：唤醒注入的"每个会话只一次"标记，**放 localStorage**。
  //   为什么不用内存变量：内存变量活不过一次页面刷新/插件重挂，而每重来一次就会再往对话里
  //   灌一整份**唤醒文案＝完整提示词**，主 agent 收到就白跑一轮、白烧 token
  //   （用户 2026-09-27 追问"开场提示词怎么发了那么多次"就是这么来的）。
  //   返回 true ＝ 这个会话已经唤醒过，本次别再灌。会话 id 变了（被归档/重建）才允许再唤醒一次。
  function claimWake(id) {
    var k = 'cfw-woke:' + String(id || '')
    try {
      if (window.localStorage && localStorage.getItem(k)) return true
