...
(function () {
  ...
  // A32（2026-09-27）：列表重绘的**数据签名**与它对应的列表节点。
  //   背景：15 秒轮询补上之后，每隔 15 秒就会 `listNode.innerHTML = ''` 把整栏 DOM 换一遍
  //   （实测一次 4 ms，本机不贵）——但**换的那一瞬间**用户若正按下/松开，mouseup 落在已被
  //   替换掉的节点上，click 根本不触发，表现就是"点一下勾选没反应 / 卡住"。
  //   现在：签名没变就只刷新不滚动的「自动采集时间」那一行，列表一个字都不动。
  var lastSig = null
  var lastSigNode = null
  ...
  // 签名要覆盖所有影响**列表渲染**的东西：条目字段、错误卡片、主 agent 提问卡片、面板分节，
  // 以及 ⋯ 菜单的展开态（菜单本来就是靠 render 开合的，漏了它菜单会点不开）。
  function renderSig() {
    try {
      var items = (store && store.items) || []
      var parts = [openMenuId,
                   (store && store.err) ? String(store.err) : '',
                   (store && store.pendingAsk) ? JSON.stringify(store.pendingAsk) : '',
                   JSON.stringify((panelCfg && panelCfg.sections) || '')]
      for (var i = 0; i < items.length; i++) {
        var x = items[i] || {}
        // 不含 __no：它由 numberInOrder 在渲染时现算并**写回条目**，收进签名会自己抖一次
        parts.push([x.id, x.kind, x.urgency, x.done ? 1 : 0, x.isNew ? 1 : 0, x.src, x.text].join('\u0001'))
      }
      return parts.join('\u0002')
    } catch (e) { return 'sig-err-' + Math.random() }
  }

  // 只重建「自动采集时间」那一行（它在不滚动的横条槽里，与列表是两个容器）
  ...
  function render() {
    if (!listNode) return
    injectStyle()
    // A32：数据没变 ⇒ 列表不动。第三个条件 `childNodes.length` 是兜底：万一别人把列表清空了
    //   （比如面板节点被重建），签名再一样也必须重画，否则会留一栏空白。
    var sig = renderSig()
    if (sig === lastSig && listNode === lastSigNode && listNode.childNodes.length) {
      renderAutoRow()
      return
    }
    lastSig = sig
    lastSigNode = listNode
