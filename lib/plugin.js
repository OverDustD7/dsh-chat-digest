...
function linkProfileIntoAgent(local) {
  if (!local) return
  const agentRoot = path.join(PKG_DIR, 'agent')
  const pairs = [
    [path.join(agentRoot, 'output'), path.join(local, 'output')],
    [path.join(agentRoot, 'docs', 'knowledge'), path.join(local, 'knowledge')],
    [path.join(agentRoot, 'docs', 'archive'), path.join(local, 'archive')],
  ]
